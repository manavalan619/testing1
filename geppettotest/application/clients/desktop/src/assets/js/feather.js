...
(function (AV) {
  AV.build = {
    ...
    feather_baseURL: "http://feather.aviary.com/3.1.0.284/",
    feather_baseURL_SSL: "https://dme0ih8comzn4.cloudfront.net/3.1.0.284/",
    ...
    api.authenticate = function () {
      ...
      return function () {
        return AV.controlsWidgetInstance.serverMessaging.sendMessage({
          ...
          onError: function () {
            AV.errorNotify("ERROR_GET_ASSETS")
          }
        })
      }
    }();
...
(function (AV, window, document) {
  AV.ServerMessaging = function (o) {
    ...
    var simpleXHRPost = function (url, method, dataObj, dataType, callback, onError) {
      return avpw$.ajax({
        url: url,
        type: method,
        data: dataObj,
    ...
    var sendMessage = function (opts) {
      ...
      if (transport === "xhr" && avpw$.support.cors && (!AV.firefox || AV.firefox >= 4)) {
        canPostXHR = simpleXHRPost(opts.action, opts.method, opts.keyValues, opts.dataType, opts.callback, opts.onError);
        if (!canPostXHR) {
...
(function (exports, window, document) {
  ...
  AV.getActiveTools = function (tools) {
    ...
    var forceCropIsOnlyAllowedTool = true;
    for (i = 0; i < activeToolsAll.length; i++) {
      ...
      if (name in supportedTools) {
        ...
        forceCropIsOnlyAllowedTool = false
      }
    }
    if (AV.launchData.forceCropPreset && forceCropIsOnlyAllowedTool) {
      AV.errorNotify("BAD_FORCECROP_TOOLS")
    }
    return activeTools
  };
  ...
  AV.paintWidgetLauncher_stage2 = function (paintImgIdElem, imgURL) {
    ...
    var loadImageFromData = function (data) {
      ...
      if (avpw$.support.cors && AV.launchData.enableCORS && data.indexOf("data:") === -1) {
    ...
    AV.controlsWidgetInstance.layoutNotify(AV.launchData["openType"], "scaleCanvas");
    if (imgURL != null) {
      if (AV.launchData.enableCORS && avpw$.support.cors && !avpw$.browser.msie && !(avpw$.browser.safari && navigator.userAgent.indexOf("Chrome") === -1 && parseInt(AV.util.getBrowserVersion()) < 7)) {
  ...
  AV.ControlsWidget.prototype.setupScrollPanels = function () {
    ...
    var _this = this;
    var PANEL_PADDING = 5;
    var lastPage, lastPageContents = {},
      pageWidth = this.layoutNotify(AV.launchData["openType"], "getDims").TOOLS_BROWSER_WIDTH;
    ...
    this.assetManager.getAssets("PERMISSION", function (assets) {
      ...
      if (aviaryBrandingShown) {
        ...
        avpw$("#avpw_controls").addClass("avpw_white_label")
      }
    });
  ...
  AV.ControlsWidget.prototype.save = function () {
    ...
    var prepareSaveMessage = function (data, overrideSaveFunction) {
      ...
      var onSaveParameters = [url, url_hires, error];
      if (typeof overrideSaveFunction === "function") {
        overrideSaveFunction.apply(this, onSaveParameters)
      } else {
        saveCallbackWithURL.apply(this, onSaveParameters)
      }
    };
    var saveImageToFeatherServices = function (onCompleteOverrideFunction) {
      ...
      _this.paintWidget.exportImage(null, function (dataURL) {
        ...
        dataURL = "";
        var originalURL = _this.origURL;
        if (originalURL && originalURL.indexOf("data:") === 0) {
          originalURL = ""
        }
    ...
    var saveImageToAsyncFeatherServices = function () {
      ...
      var saveHiRes = function () {
        _hiResSaveTracking(true, dims.hiresWidth, dims.hiresHeight);
        if (!AV.launchData.hiresUrl) {
          AV.errorNotify("ERROR_MISSING_HI_RES_URL");
          return
        }
        ...
        var getJobStatusCallback = function (d) {
          if (!d || d && d.JobStatusCode === "JobFailed") {
            hiresSaveCallbackWithURL(null, d);
      ...
      var save = function () {
        saveImageToFeatherServices.apply(_thisContext, [function (data, status) {
          _hiResSaveTracking(false, dims.hiresWidth, dims.hiresHeight);
          prepareSaveMessage.apply(this, [data, hiresSaveCallbackWithURL])
        }])
      };
      var isIE = navigator.userAgent.indexOf("MSIE") !== -1;
      if (isIE || AV.launchData.forceHiResSave || AV.launchData.hiresUrl !== AV.launchData.url) {
        saveHiRes.call(this)
      } else {
        var dims = AV.paintWidgetInstance.getScaledSize();
        var maxSize = AV.launchData.maxSize;
        var width = AV.launchData.hiresWidth;
        var height = AV.launchData.hiresHeight;
        if (width && height) {
          if (width > maxSize && height > maxSize) {
            saveHiRes.call(this)
          } else {
            if (dims.hiresWidth < maxSize && dims.hiresHeight < maxSize) {
              save.call(this)
            } else {
              saveHiRes.call(this)
            }
...
(function (exports, window, document) {
  ...
  baseConfig.forceHiResSave = false;
  baseConfig.authenticationURL = null;
  ...
  AV.Feather = function (config) {
    ...
    if (config.authenticationURL) {
      config.isPremiumPartner = true;
      mediator.on("auth:update", function (callback) {
        _getAuthenticationSignature(callback)
      })
    }
    var _getAuthenticationSignature = function (callback) {
      avpw$.ajax({
        ...
        var requiredProperties = ["salt", "timestamp", "signature", "encryptionMethod"];
        var missingProperties = [];
        var prop;
        for (var i = 0; i < requiredProperties.length; i++) {
          prop = requiredProperties[i];
          if (!data[prop]) {
            missingProperties.push(prop)
          }
        }
        if (missingProperties.length > 0) {
          AV.errorNotify("ERROR_BAD_AUTHENTICATION_PARAMETERS", [missingProperties.join(", ")])
        }
        api.updateConfig({
          salt: data.salt,
    ...
    var AV__featherAutoInit = function () {
      ...
      AV.featherUseFlash = !AV.featherHtmlOk() && AV.featherFlashOk();
      if (AV.featherUseFlash) {
        AV.launchData.forceHiResSave = true
      }
      AV.launchData["language"] = AV.launchData["language"].toLowerCase();
      if (!AV.feather_loaded && !AV.feather_loading) {
        AV.feather_loading = true;
    ...
    api.launch = function (launchData) {
      ...
      if (AV.launchData && (AV.launchData.hiresWidth || AV.launchData.hiresHeight)) {
        AV.launchData.hiresWidth = null;
        AV.launchData.hiresHeight = null
      }
    ...
    api.saveHiRes = function () {
      ...
      if (!AV.launchData["authenticationURL"]) {
        return AV.controlsWidgetInstance.save("saveHiRes")
      } else {
        mediator.trigger("auth:update", function () {
          AV.controlsWidgetInstance.save("saveHiRes")
        });
        return true
      }
    };
  ...
  AV.loadStageFinal = function () {
    ...
    var checkOnloadAndRun = function () {
    ...
    if (!AV.launchData["authenticationURL"]) {
      ...
      mediator.trigger("auth:update", checkOnloadAndRun)
    }
  };
